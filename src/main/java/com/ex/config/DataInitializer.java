...
public class DataInitializer implements CommandLineRunner {
    ...
    private Product createProduct(
        ...
        int saleSampleDays = switch (lotNumber) {
            case "FF-HB-260721" -> 40; // 10% 할인 예시
            case "FF-PG-260724" -> 20; // 20% 할인 예시
            case "FF-CK-260716" -> 10; // 30% 할인 예시
            case "FF-MN-260715" -> 6;  // 40% 할인 예시
            default -> 0;
        };
        LocalDate expirationDate = saleSampleDays > 0
                ? LocalDate.now().plusDays(saleSampleDays)
                : LocalDate.now().plusMonths(expirationMonths);
